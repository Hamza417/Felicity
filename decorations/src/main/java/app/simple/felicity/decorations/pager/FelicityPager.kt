 ...
 * - Selection events: [onPageSelected] is dispatched only after the final settle completes (or immediately
 ...
 * - fromUser flag: An overload [onPageSelected] with (position, fromUser) is provided to differentiate user-initiated
 *   vs app-initiated changes. Both the new overload and the legacy [onPageSelected] are called for
 *   compatibility. User drags/flings pass fromUser = true; programmatic navigation passes false.
 * - State callbacks: [onPageScrollStateChanged] is dispatched with [SCROLL_STATE_DRAGGING], [SCROLL_STATE_SETTLING], and [SCROLL_STATE_IDLE]. Dragging starts
