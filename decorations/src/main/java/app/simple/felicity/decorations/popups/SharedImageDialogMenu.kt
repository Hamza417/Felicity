...
abstract class SharedImageDialogMenu<VB : ViewBinding> @JvmOverloads constructor(
    ...
    companion object {
        ...
        private const val SCRIM_COLOR = "#99000000"
        const val DEFAULT_WIDTH_RATIO = 0.75f // 75% of screen width
    ...
    private fun animateShow() {
        ...
        // Content fade in (starts earlier, overlaps with image animation)
        val contentAnimator = ObjectAnimator.ofFloat(contentContainer, View.ALPHA, 0f, 1f).apply {
            duration = (DURATION * 0.6).toLong()
            startDelay = (DURATION * 0.3).toLong()
            interpolator = EMPHASIZED_DECELERATE
        }

        AnimatorSet().apply {
            playTogether(scrimAnimator, xAnimator, yAnimator, widthAnimator, heightAnimator, cornerAnimator, contentAnimator)
    ...
    fun dismiss() {
        ...
        // Content fade out (quick)
        val contentAnimator = ObjectAnimator.ofFloat(contentContainer, View.ALPHA, 1f, 0f).apply {
            duration = (DURATION * 0.3).toLong()
            interpolator = EMPHASIZED_ACCELERATE
        }
        ...
        AnimatorSet().apply {
            playTogether(scrimAnimator, contentAnimator, xAnimator, yAnimator, widthAnimator, heightAnimator, cornerAnimator)
            addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    cleanup()
                }
