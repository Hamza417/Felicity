...
/**
 * A [DynamicRippleLinearLayout] that automatically registers itself with
 * [MediaPlaybackManager] to reflect the currently playing song state. When the
 * playing song changes, the selection highlight transitions smoothly via a color
 * animation.
 *
 ...
 * Think of it as a little sticky note on the right side that says "yep, this one's picked!"
 *
 ...
 * @author Hamza417
 */
class MediaAwareRippleLinearLayout @JvmOverloads constructor(
        context: Context, attrs: AttributeSet? = null
) : DynamicRippleLinearLayout(context, attrs), MediaStateListener {
     ...
     * Called from [onSizeChanged] and whenever the accent color changes lazily in [onDraw].
     */
     ...
     * behind all the child views (text, icons, etc.). It fades in from transparent
    ...
    companion object {
        @Suppress("unused")
        private const val TAG = "MediaAwareRippleLinearLayout"
    }
}