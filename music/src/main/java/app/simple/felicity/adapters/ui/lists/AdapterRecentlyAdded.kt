...
/**
 * Recycler adapter for the Recently Added panel. Identical in structure to [AdapterSongs]
 * but reads [RecentlyAddedPreferences.getGridSize] for its initial layout mode, keeping
 * the Recently Added grid/list setting fully independent from the Songs panel.
 *
 * @author Hamza417
 */
class AdapterRecentlyAdded(initial: List<Audio>) : FastScrollAdapter<VerticalListViewHolder>() {
    ...
    var layoutMode: CommonPreferencesConstants.LayoutMode = RecentlyAddedPreferences.getGridSize()
