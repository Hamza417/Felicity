...
import app.simple.felicity.engine.processors.BassProcessor.Companion.FLAT_THRESHOLD_DB
import app.simple.felicity.engine.processors.BassProcessor.Companion.SHELF_FREQUENCY_HZ
...
/**
 * A custom [AudioProcessor] that applies a second-order low-shelf biquad IIR filter for bass
 * tone control, using the RBJ Audio EQ Cookbook low-shelf formula with shelf slope S = 1.0.
 *
 * The shelf frequency is fixed at [SHELF_FREQUENCY_HZ] (250 Hz), giving a broad bass boost or
 ...
 *
 * RBJ low-shelf coefficients (S = 1):
 * ```
 *   A  = 10^(dBgain / 40)
 *   w0 = 2π × f0 / Fs
 *   α  = sin(w0) / √2
...
class BassProcessor : AudioProcessor {
    ...
    /**
     * Normalized biquad coefficients [b0, b1, b2, a1, a2] for the low-shelf filter.
    ...
    /**
     * Sets the bass shelf gain and immediately recomputes the biquad coefficients.
     *
    ...
    /** Returns the current bass gain in dB. */
    ...
    /**
     * Processes each PCM frame through the low-shelf biquad filter.
    ...
    /**
     * Computes normalized low-shelf biquad coefficients for [fs] Hz sample rate using
     * the RBJ low-shelf formulas at shelf slope S = 1.
     *
    ...
    companion object {
        /** Shelf corner frequency in Hz. Frequencies below this are boosted or cut. */
        private const val SHELF_FREQUENCY_HZ = 250.0
